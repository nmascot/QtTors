...
\fun{ulong}{maxprimelim}{} the argument used for the last \kbd{initprimes}
...
\fun{GEN}{RgM_Frobenius}{GEN A, long flag, GEN* P, GEN* v} given a square matrix $A$, returns the Frobenius form of A if $flag$ is 0, or a weak Frobenius form of $A$ if $flag$ is 1. If $P$ is nonzero, set it to the change of basis matrix. If $v$ is nonzero, set it to the Vecsmall containing the column indices of the left column of each diagonal block.
