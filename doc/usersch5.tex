...
Namely an integer $0 < \kbd{pi} < B$ such that, given $0 \leq x < B^2$ (by
...
once we set $q = 2^k p$ for the unique $k$ such that $B/2 \leq q < B$, it is
equal to the Euclidean quotient of $B^2 - qB + B-1$ by $q$. In particular
$(\kbd{pi} + B) / B^2$ is very close to $1/q$.
...
assuming \kbd{pi} is the pseudoinverse of $p$.
...
modulo $p$, assuming \kbd{pi} is the pseudoinverse of $p$.

\fun{ulong}{Fl_addmul_pre}{ulong a, ulong b, ulong c, ulong p,  ulong pi}
returns $a+b\*c$ modulo $p$, assuming \kbd{pi} is the pseudoinverse of $p$.

\fun{ulong}{Fl_addmulmul_pre}{ulong a,ulong b, ulong c,ulong d, ulong p, ulong pi}
returns $a\*b+c\*d$ modulo $p$, assuming \kbd{pi} is the pseudoinverse of $p$.

\fun{ulong}{Fl_powu_pre}{ulong x, ulong n, ulong p, ulong pi} returns
$x^n$ modulo $p$, assuming \kbd{pi} is the pseudoinverse of $p$, or $0$
...
the vector (\typ{VECSMALL}) $(x^0, \dots, x^n)$, assuming \kbd{pi} is
...
as \kbd{Fl\_log}, assuming \kbd{pi} is the pseudoinverse of $p$, or $0$ in
...
of $x$ modulo $p$, see \kbd{Fl\_sqrt}. We assume \kbd{pi} is the
...
\kbd{p}. We assume \kbd{pi} is the pseudoinverse of $p$, or $0$ in which case
...
See \kbd{Fl\_sqrtn}, assuming \kbd{pi} is the pseudoinverse of $p$, or $0$ in
which case we either use ordinary divisions if $p < B^{1/2}$ is small and
call \kbd{get\_Fl\_red} ourselves otherwise.
...
We assume \kbd{pi} is the pseudoinverse of $p$, or $0$ in which case we
...
\kbd{Fl\_2gener\_pre}. We assume \kbd{pi} is the pseudoinverse of $p$, or $0$
...
and \kbd{y} (assumed to have compatible dimensions), assuming \kbd{pi} is the
...
\kbd{Flm} \kbd{x} by \kbd{y} assuming \kbd{pi} is the pseudoinverse of $p$,
...
scalar product of \kbd{x} and \kbd{y} assuming \kbd{pi} is the pseudoinverse of
...
\kbd{y} (assumed to have compatible dimensions), assuming \kbd{pi} is the
...
\fun{GEN}{Flx_mul_pre}{GEN x, GEN y, ulong p, ulong pi}. We assume \kbd{pi} is
...
\fun{GEN}{Flx_sqr_pre}{GEN x, ulong p, ulong pi}. We assume \kbd{pi} is the
pseudoinverse of $p$, or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
here $p$ must be prime. We assume \kbd{pi} is the pseudoinverse of $p$, or
$0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
\fun{GEN}{Flx_rem_pre}{GEN x, GEN y, ulong p}, as \kbd{Flx\_rem}.
We assume \kbd{pi} is the pseudoinverse of $p$, or $0$ in which case we
assume \kbd{SMALL\_ULONG}$(p)$.
...
as \kbd{Flx\_Frobenius}. We assume \kbd{pi} is the pseudoinverse of $p$, or
$0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
\fun{GEN}{Flx_gcd_pre}{GEN a, GEN b, ulong p} as \kbd{Flx\_gcd}. We assume
\kbd{pi} is the pseudoinverse of $p$, or $0$ in which case we assume
\kbd{SMALL\_ULONG}$(p)$.
...
\fun{GEN}{Flx_halfgcd_pre}{GEN a, GEN b, ulong p} as \kbd{Flx\_halfgcd}. We
assume \kbd{pi} is the pseudoinverse of $p$, or $0$ in which case we assume
\kbd{SMALL\_ULONG}$(p)$.
...
as \kbd{Flx\_extgcd}. We assume \kbd{pi} is the pseudoinverse of $p$, or $0$
in which case we assume \kbd{SMALL\_ULONG}$(p)$.

\fun{GEN}{Flx_roots}{GEN f, ulong p} returns the vector of roots
of $f$ (without multiplicity, as a \typ{VECSMALL}). Assumes that $p$ is
prime.

\fun{GEN}{Flx_roots_pre}{GEN f, ulong p}, as \kbd{Flx\_roots}. We assume
\kbd{pi} is the pseudoinverse of $p$, or $0$ in which case we assume
\kbd{SMALL\_ULONG}$(p)$.
...
\fun{GEN}{Flx_oneroot_pre}{GEN f, ulong p}, as \kbd{Flx\_oneroot}. We assume
\kbd{pi} is the pseudoinverse of $p$, or $0$ in which case we assume
\kbd{SMALL\_ULONG}$(p)$.
...
\fun{ulong}{Flx_oneroot_split_pre}{GEN f, ulong p, ulong pi} as
\kbd{Flx\_oneroot\_split}. We assume \kbd{pi} is the pseudoinverse of $p$, or
$0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
\fun{ulong}{Flx_eval_pre}{GEN x, ulong y, ulong p, ulong pi}, as
\kbd{Flx\_eval}. We assume \kbd{pi} is the pseudoinverse of $p$,
or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
\kbd{Flx} $P$, return $P(a)$. We assume \kbd{pi} is the pseudoinverse of $p$,
or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
as \kbd{Flx\_Flxq\_eval}, where \kbd{pi} is a pseudoinverse of $p$,
or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
see \kbd{FlX\_FlxqV\_eval}. We assume \kbd{pi} is a pseudoinverse of $p$, or
$0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
as \kbd{FlxC\_Flxq\_eval}. We assume \kbd{pi} is a pseudoinverse of $p$, or
$0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
...
as \kbd{FlxC\_FlxqV\_eval} where \kbd{pi} is a pseudoinverse of $p$,
or $0$ in which case we assume \kbd{SMALL\_ULONG}$(p)$.
