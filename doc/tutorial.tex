...
as a first try, we type \kbd{qfbpow(f,621)} (the direct command \kbd{f \pow
621} does not accept extended forms). Oops, we overshoot: the distance is now
$3173.018\dots > R$. Now we can refine our initial estimate: we should be
close to the correct distance if we raise \kbd{f} to the power $621*R/3173$
which is a little less than $517$. Now if we compute \kbd{qfbpow(f, 517)} we
hit the principal form right on the dot (actually, 516 would also work!).
Note that this is not a lucky accident: we must land extremely close to the
correct target using this method, and usually at most one reduction
correction step is necessary. And the distance component can tell us where we
are along the cycle.
...
digits, we retype the definition of \kbd{u} (why is this necessary?), and then
